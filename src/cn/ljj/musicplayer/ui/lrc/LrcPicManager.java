...
public class LrcPicManager {
    ...
    private static LrcPicManager sInstance = null;
    private PlayList mPlayList = null;

    private LrcPicManager(Context context) {
        mPlayList = PlayList.getPlayList(context);
    }

    public static LrcPicManager getInstance(Context context) {
        if (sInstance == null) {
            sInstance = new LrcPicManager(context);
        }
        return sInstance;
    }

    public String getLrc(final MusicInfo music) {
    ...
    private void downloadLrc(final MusicInfo music, String link) {
        String saveName = music.getName() + ".Lrc";
        DownloadFactory.DownloadLrc(link, saveName, new DownloadCallback() {
            @Override
            public void onProgressChange(int length, int finished) {
                Logger.v(TAG, "downloadPic onFinished finished " + ((finished * 100) / length)
                        + "%");
            }
    ...
    public String getPic(final MusicInfo music) {
    ...
    private void downloadPic(final MusicInfo music, String link) {
